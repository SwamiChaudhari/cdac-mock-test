    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      ...
      <header className="bg-black/30 backdrop-blur-sm border-b border-white/10 p-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center text-xl font-bold shadow-lg shadow-orange-500/30">
              {level}
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">{levelProgress.current}/{levelProgress.needed} XP to Level {level + 1}</p>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex gap-2 mb-6">
          {(['overview', 'quests', 'achievements', 'flashcards'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-lg font-medium text-sm capitalize transition ${activeTab === tab ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  {Object.entries(subjectPerf).sort((a, b) => b[1].total - a[1].total).map(([subject, data]) => {
                    ...
                    return (
                      <div key={subject} className="flex items-center gap-3">
                        <div className="w-32 text-sm text-gray-300 truncate">{subject}</div>
                        ...
                        <div className="w-12 text-right text-sm font-medium">{acc}%</div>
                      </div>
                    )
                  })}
                  {Object.keys(subjectPerf).length === 0 && <p className="text-gray-500 text-center py-4">Take some tests to see performance</p>}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {recentResults.map((r) => (
                    <div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                      <div>
                        <p className="text-sm font-medium">{r.mode}</p>
                        <p className="text-xs text-gray-400">{new Date(r.date).toLocaleDateString()}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold">{r.score}/{r.totalMarks}</p>
                        <p className="text-xs text-gray-400">{Math.round(r.accuracy)}% acc</p>
                      </div>
                    </div>
                  ))}
                  {recentResults.length === 0 && <p className="text-gray-500 text-center py-4">No tests yet</p>}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {/* Due Flashcards */}
              <div className="bg-white/5 rounded-xl border border-white/10 p-6">
                <h3 className="text-lg font-semibold mb-3">📚 Due for Review</h3>
                <p className="text-3xl font-bold text-purple-400">{dueFlashcards.length}</p>
                <p className="text-xs text-gray-400 mt-1">flashcards ready</p>
              </div>
        ...
        {activeTab === 'quests' && (
          <div className="space-y-4">
            ...
            {dailyQuests.map(quest => (
              <div key={quest.id} className={`p-4 rounded-xl border ${quest.completed ? 'bg-green-900/30 border-green-500/30' : 'bg-white/5 border-white/10'}`}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    {quest.completed && <p className="text-green-400 text-sm">✓ Done</p>}
                  </div>
                </div>
                <div className="mt-2 h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div className="h-full bg-purple-500" style={{ width: `${Math.min(100, (quest.current / quest.target) * 100)}%` }}></div>
                </div>
                <p className="text-xs text-gray-500 mt-1">{quest.current}/{quest.target}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab === 'achievements' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ACHIEVEMENTS.map(a => {
              ...
              return (
                <div key={a.id} className={`p-4 rounded-xl border ${unlocked ? 'bg-yellow-900/30 border-yellow-500/30' : 'bg-white/5 border-white/10 opacity-50'}`}>
        ...
        {activeTab === 'flashcards' && (
          <div>
            <h2 className="text-xl font-bold mb-4">Flashcards ({flashcards.length})</h2>
            {dueFlashcards.length > 0 && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-purple-400 mb-3">Due for Review ({dueFlashcards.length})</h3>
                <div className="space-y-3">
            ...
            {flashcards.length === 0 && (
              <p className="text-gray-500 text-center py-8">No flashcards yet. Create some during practice!</p>
            )}
          </div>
        )}
      </div>
...
function FlashcardItem({ card }: { card: any }) {
  ...
  return (
    <div className="bg-white/5 rounded-xl border border-white/10 p-4 cursor-pointer" onClick={() => setFlipped(!flipped)}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <p className="text-xs text-purple-400 mb-1">{card.topic}</p>
          {flipped ? (
            <div>
